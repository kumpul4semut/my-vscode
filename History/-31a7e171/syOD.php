...
class Kontak extends ADMIN_Controller
{
	...
	public function index()
	{
		...
		$this->data['jumlah_kontak'] = @$this->kontak->kontak_has_donasi() ? $this->kontak->kontak_has_donasi()->num_rows() : 0;
	...
	public function get_activity()
	{
		...
		foreach ($perday as $k => $v) {
			...
			$perday[$k]['activity_detail'] = $this->activity->with_user()->with_activity_type()->where($where)->length($length)->start($start)->order('activity.date_created', 'desc')->get($select_field)->result_array();
