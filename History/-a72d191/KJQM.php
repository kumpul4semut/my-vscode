...
class Kontak extends ADMIN_Controller
{
	...
	public function get_activity()
	{
		...
		foreach ($perday as $k => $v) {
			...
			$perday[$k]['activity_detail'] = $this->activity->with_user()->with_activity_type()->where($where)->or_where($or_where)->length($length)->start($start)->order('activity.date_created', 'desc')->group('activity.id')->get($select_field)->result_array();
